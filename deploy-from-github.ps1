...
ssh "${NAS_USER}@${NAS_HOST}" "mkdir -p /volume1/web && cd /volume1/web && rm -rf c4-mcp-app && GIT_TERMINAL_PROMPT=0 git clone $GITHUB_REPO 2>&1 || echo 'Clone failed - repository may be private'"
...
ssh "${NAS_USER}@${NAS_HOST}" "cd $NAS_DIR/backend && cp .env.example .env && chmod 600 .env"
